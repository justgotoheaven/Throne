...
EditAmneziaWG::EditAmneziaWG(QWidget *parent) : QWidget(parent), ui(new Ui::EditAmneziaWG) {
    ui->setupUi(this);
}

EditAmneziaWG::~EditAmneziaWG() {
    delete ui;
}

void EditAmneziaWG::onStart(std::shared_ptr<Configs::Profile> _ent) {
    this->ent = _ent;
    auto outbound = this->ent->AmneziaWG();
